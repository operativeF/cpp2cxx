...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        config_file_options.add_options()("help,h", "produce help message")("verbose,v",
                ...
                "input-file,i", po::value<std::vector<std::filesystem::path>>(&input_files),
                "input file")
                ...
                ("output-file,o", po::value<std::vector<std::filesystem::path>>(&output_files),
                        ...
                        "validator-file,s",
                        po::value<std::filesystem::path>(&validator_file)
                                ->default_value("Defined.h"),
                        ...
                        po::value<std::filesystem::path>(&output_directory)
                                ->default_value("dm_dir"),
                        "the directory where translated files should be put to")(
                        "cleanup-directory",
                        po::value<std::filesystem::path>(&cleanup_directory)
                                ->default_value("cleanup"),
                        "the directory where (finally) demacrofied files should be put to")(
                        "backup-directory",
                        po::value<std::filesystem::path>(&backup_directory)->default_value("."),
        ...
        if(!cfg.is_open())
        {
            fmt::print(std::cerr, "Could not open config file: {}\n", config_file);
            return -1;
        }
    ...
    po::store(po::command_line_parser(argc, argv).
      ...
      */
        ...
        {
            fmt::print(
                    "Warning, no output filename specified."
        ...
        pConfigScheme.SetDemacroficationScheme(
                demacrofication_granularity, macros_preventing_demacrofication, enable_warning,
                global_macros_raw, global_macros_formatted, multiple_definitions,
    ...
    {
        fmt::print(std::cerr, "Exception Handler: {}", e.GetMessage());
    }
